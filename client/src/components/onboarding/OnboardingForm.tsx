...
const OnboardingForm = () => {
  ...
  const onSubmit = (data: OnboardingFormValues) => {
    ...
    // Use the mutation approach which handles loading state
    onboardingMutation.mutate(completeData, {
      onSuccess: () => {
        // Force navigation to the search page after success
        console.log("onboardingMutation success, redirecting to /search");
        // Use a timeout to ensure the UI has a chance to update
        setTimeout(() => {
          window.location.href = "/search";
        }, 500);
      }
